...
export default function BudgetsPage() {
  ...
  const filteredBudgets = activeTab === t.yearly 
    ? budgets 
    : budgets.filter(b => b.month === new Date().getMonth() + 1);

  const totalUnder = filteredBudgets.filter(b => b.spent < b.limit).length;
  const totalOver = filteredBudgets.filter(b => b.spent >= b.limit).length;
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white">{t.budgets}</h1>
        ...
        >
          ...
          <span className="font-semibold">{t.createNewBudget}</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="p-6 rounded-3xl border border-white/5 bg-gradient-to-br from-emerald-500/10 to-transparent glass-dark">
          ...
          <h3 className="text-slate-400 text-sm font-medium">{t.underBudget}</h3>
          <p className="text-2xl font-bold text-white mt-1">{totalUnder} {t.categoriesLabel}</p>
        </div>
        <div className="p-6 rounded-3xl border border-white/5 bg-gradient-to-br from-rose-500/10 to-transparent glass-dark">
          ...
          <h3 className="text-slate-400 text-sm font-medium">{t.overBudgetLabel}</h3>
          <p className="text-2xl font-bold text-white mt-1">{totalOver} {totalOver === 1 ? t.categoryLabel : t.categoriesLabel}</p>
        </div>
        <div className="p-6 rounded-3xl border border-white/5 bg-gradient-to-br from-primary/10 to-transparent glass-dark">
          ...
          <h3 className="text-slate-400 text-sm font-medium">{t.averageSavings}</h3>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-sm text-slate-400">{t.totalLimitLabel}:</span>
                  ...
                  <div className="h-3 w-full bg-slate-800 rounded-full overflow-hidden p-0.5 border border-white/5">
                    <div 
                      className={cn(
                        ...
                        budget.bgColor || (isOver ? "bg-rose-500" : "bg-emerald-500")
                      )}
